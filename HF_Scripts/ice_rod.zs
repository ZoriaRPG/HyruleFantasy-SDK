...
const int TILE_ICE_CUBE = 21282;//Tile to draw on top of frozen enemy.
...
ffc script IceRod{
	void run (int damage, int wandsprite, int sprite, int freezeduration, int speed, int cost){
		...
		MeleeWeaponSlash90Alt(this, wand, wand->Misc[LWEAPON_MISC_ORIGTILE], 8, 12, CF_ICE_ROD_DIRECT_TRIGGER, 0, NPCD_WAND, 0, 2);
...
ffc script IceRodMagic{
	void run (int damage, int sprite, int freezeduration, int speed){
		...
		while(true){
			...
			if ((WallCollision8way(l))||(BlockedByEnemy(l))||(l->X < 15 || l->X > 241 || l->Y < 15 || l->Y > 161))
			{
				TerminateLweapon (this, l);
			}
			if (LweaponFlagIsSet(l, LWF_REFLECTED))TriggerUpdate (l, CF_REFMAGIC, SFX_SECRET, true);		
			TriggerUpdate2 (l, CF_ICE_ROD_MAGIC_TRIGGER, SFX_SECRET, true);
