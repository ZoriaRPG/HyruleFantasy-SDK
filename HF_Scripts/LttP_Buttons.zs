const int SFX_SWITCH_PRESS = 66; //SFX when a switch is pressed
const int SFX_SWITCH_RELEASE = 66; //SFX when a switch is released
const int SFX_SWITCH_ERROR = 88; //SFX when the wrong switch is pressed
...
int Switch_Pressed(int x, int y){
	...
	for(int i=1; i<=32; i++){
		...
		if(f->Script==ffcs){
			if(Abs(f->X-x)<=8&&Abs(f->Y+16-y)<=8&&f->InitD[0]==0)
				return 1;
		}
	}
	return 0;
