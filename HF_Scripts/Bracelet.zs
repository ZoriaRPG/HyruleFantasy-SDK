...
void PowerBracelet(){
if(Link->Item[LTM_HOLDING] || Link->Item[LTM_CATCHING] || Link->Item[LTM_PULLING]){
...
void DoPowerBracelet()
{
	if (Link->PressEx2 && (Link->Action == LA_NONE || Link->Action == LA_WALKING))
	{
		...
		{ // use this line if not using GB_Shop and GB_Bombs
			if(!powerBracelet[LINK_CATCHING] && (isSolid(TouchedX(),TouchedY()) || PowerBracelet_GrabbingElephantStatue() || PowerBracelet_GrabbingBeamosStatue() || PowerBracelet_GrabbingLever()))
			{
				powerBracelet[LINK_CATCHING]=1;
				int args[] = {0,0,0,0,0,0,0,0};
				int id = RunFFCScript(SCRIPT_POWERBRACELET, args);
...
ffc script UsePowerBracelet{
	void run(int input){
		...
		while(Link->InputA || Link->InputB || Link->InputEx2){
			...
			int grabbingBeamos = PowerBracelet_GrabbingBeamosStatue();
			int grabbingLever = PowerBracelet_GrabbingLever();
			
			// to fix a bug...
			if(!isSolid(TouchedX(),TouchedY()) && grabbingStatue == 0 && grabbingBeamos == 0 && grabbingLever == 0) break;
			...
			if (grabbingLever > 0 && counter > 0)
			{
				statue = Screen->LoadFFC(grabbingLever);
				statue->InitD[0] = 1;
				while (powerBracelet[LINK_CATCHING])
				{
					Link->X = statue->X;
					Link->Y = statue->Y + 12;
					Link->Dir = DIR_UP;
			...
			if(counter>PB_PULL_TIME){
				...
				else if(grabbingBeamos>0){ //Grabbing a statue overrides everything else
					if(!Link->Item[56])
						canLift = false;
					statue = Screen->LoadFFC(grabbingBeamos);
					if(statue->InitD[1]>=1) //Can't lift a statue that's already lifted
						canLift = false;
					LinkMovement_Push2NoEdge(statue->InitD[2], statue->InitD[3]);
				...
				if(canLift){
					...
					else if(grabbingBeamos>0){
						powerBracelet[HOLDING_BUSH] = 0;
						statue = Screen->LoadFFC(grabbingBeamos); //Warning: Pointless ahead. Message, and then...Message? Try jumping.
						combo = statue->InitD[5];
						cset = statue->InitD[6];
						statue->InitD[1] = 1; //Tell the statue to disappear
					}
					else{
					...
					if(grabbingStatue>0){
						...
						for(int i=0;i<16;i++){
							...
							this->InitD[1] = blockX;
							this->InitD[2] = blockY;
							int EyeX = blockX+VectorX(8, statue->InitD[4]);
							int EyeY = blockY+4+VectorY(5, statue->InitD[4]);
							int EyeCombo = combo+2+AngleDir8(statue->InitD[4]);
							if(i!=0) //Disable draw for the first frame since the statue needs a frame to disappear
							{
								if(statue->InitD[4]<=0)
									Screen->FastCombo(LAYER_BEAMOS2, EyeX, EyeY, EyeCombo, cset, 128);
								Screen->FastCombo(LAYER_BEAMOS2, blockX, blockY, combo, cset, 128);
								Screen->FastCombo(LAYER_BEAMOS2, blockX, blockY+16, combo+1, cset, 128);
								if(statue->InitD[4]>0)
									Screen->FastCombo(LAYER_BEAMOS2, EyeX, EyeY, EyeCombo, cset, 128);
							}
							cset = statue->InitD[6];
					...
					while(powerBracelet[THROW_DISABLED] || (!Link->InputA && !Link->InputB && !Link->InputEx2)){
						...
						{
							int EyeX = Link->X+VectorX(8, statue->InitD[4]);
							int EyeY = Link->Y-28+4+VectorY(5, statue->InitD[4]);
							this->InitD[1] = Link->X;
							this->InitD[2] = Link->Y - 28;
							cset = statue->InitD[6];
							int EyeCombo = combo+2+AngleDir8(statue->InitD[4]);
							if(statue->InitD[4]<=0)
								Screen->FastCombo(LAYER_BEAMOS2, EyeX, EyeY, EyeCombo, cset, 128);
							Screen->FastCombo(LAYER_BEAMOS2, Link->X, Link->Y-28, combo, cset, 128);
							Screen->FastCombo(LAYER_BEAMOS2, Link->X, Link->Y-28+16, combo+1, cset, 128);
							if(statue->InitD[4]>0)
								Screen->FastCombo(LAYER_BEAMOS2, EyeX, EyeY, EyeCombo, cset, 128);
						}
						else
					...
					if(Link->Invisible || (Link->Action != LA_NONE && Link->Action != LA_WALKING)){
						if(grabbingStatue==0 && grabbingBeamos==0)
					...
					if(grabbingStatue>0){ //If Link is holding a statue, the throwing code is different
					...
					else if(grabbingBeamos>0){ //If Link is holding a statue, the throwing code is different
						int statueX = Link->X;
						int statueY = Link->Y;
						int statueZ = 12;
						int step = 3;
						...
						while(statueZ>0||bounces>0){
							...
							int EyeX = statueX+VectorX(8, statue->InitD[4]);
							int EyeY = statueY - statueZ - 16+4+VectorY(5, statue->InitD[4]);
							int EyeCombo = combo+2+AngleDir8(statue->InitD[4]);
							if(statue->InitD[4]<=0)
								Screen->FastCombo(LAYER_BEAMOS2, EyeX, EyeY, EyeCombo, cset, 128);
							Screen->FastCombo(LAYER_BEAMOS2, statueX, statueY - statueZ - 16, combo, cset, 128);
							Screen->FastCombo(LAYER_BEAMOS2, statueX, statueY - statueZ, combo+1, cset, 128);
							if(statue->InitD[4]>0)
								Screen->FastCombo(LAYER_BEAMOS2, EyeX, EyeY, EyeCombo, cset, 128);
							this->InitD[1] = statueX;
							this->InitD[2] = statueY - statueZ - 16;
							Waitframe();
						}
						if(hitbox->isValid())
							hitbox->DeadState = 0;
...
int PowerBracelet_GrabbingBeamosStatue(){
	int str[] = "Beamos2";
	int ffcs = Game->GetFFCScript(str);
	int x; int y;
	if(Link->Dir==DIR_UP){
	...
	for(int i=1; i<=32; i++){
		ffc f = Screen->LoadFFC(i);
		if(f->Script==ffcs){
			if(x>=f->X&&x<=f->X+15&&y>=f->Y+16&&y<=f->Y+31&&f->InitD[1]==0)
				return i;
		}
	}
	return 0;
...
int PowerBracelet_GrabbingLever(){
	int str[] = "Lever";
	int ffcs = Game->GetFFCScript(str);
	int x; int y;
	if(Link->Dir==DIR_UP){
	...
	for(int i=1; i<=32; i++){
		ffc f = Screen->LoadFFC(i);
		if(f->Script==ffcs){
			if(x>=f->X&&x<=f->X+15&&y>=f->Y&&y<=f->Y+15&&f->InitD[0]==0)
				return i;
		}
	}
	return 0;
